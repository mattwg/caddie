...
### Per-notebook dependency isolation

Every generated notebook carries its own PEP 723 (`# /// script`) inline dependency header, declaring caddie itself (as a local `file://` dependency on the checkout it's running from) plus caddie's own runtime dependencies. `/caddie-edit` opens the notebook via `marimo edit --sandbox`, which resolves that header into an isolated `uv`-managed environment instead of running against caddie's own shared venv. `/caddie-add-dependency <project> <package>` adds a package to one notebook's own header (via `uv add --script`), touching only that notebook — never caddie's own `pyproject.toml`, and never another notebook's header.

This keeps a package one notebook's analysis code needs from ever having to be reconciled against caddie core's own dependencies, or against what any other notebook needs — each notebook's environment is resolved independently. `/caddie-ask`'s own guided step execution is the one exception: it runs in-process against caddie's shared venv, not sandboxed, since it has to share a live, unpicklable connector session across steps within an episode. A package added via `/caddie-add-dependency` is available the next time that notebook opens in `/caddie-edit`, not retroactively inside an in-progress `/caddie-ask` episode.

Implementation note: caddie is declared as a plain `file://` URL dependency rather than an editable `[tool.uv.sources]` path. `marimo edit --sandbox` resolves a script's dependencies via `uv export --no-header --script`, which (as of uv 0.11.5) mis-serializes a local editable requirement as a bare `-e` line with no path, breaking the sandbox — a `file://` URL exports cleanly. The tradeoff is that a sandbox picks up caddie's source as of whenever it was last resolved, not live; adding a dependency (or running `uv lock --script` directly) forces a rebuild. A `file://` (or future pinned git) dependency also means the notebook only runs on a machine that can resolve caddie itself — see "Portable notebooks" below for the case where that's not acceptable.

### Portable notebooks

Even with the isolation above, the working notebook still depends on the `caddie` package itself for its `setup` cell (reading `caddie.yaml`, resolving the connector through caddie's plugin loader, applying the shared chart template) — it is not a hermetic script, the same way a company-internal analytics library's user isn't hermetic either. `/caddie-share <project>` (backed by `caddie notebook-share`) produces `notebook.portable.py` alongside the working notebook: the same cells, but with the `setup` cell rewritten to vendor the literal source of the connector plugin actually in use (every built-in connector is caddie-import-free by design, so its module source runs standalone) plus the charting template's source, instantiating the connector directly from the project's saved settings instead of re-reading `caddie.yaml`. The vendored cell is marked `hide_code` so it doesn't clutter the notebook when opened. The portable file's own PEP 723 header drops `caddie` and `ruamel-yaml` (only needed to read `caddie.yaml`) from the dependency list.
